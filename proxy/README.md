...
Allowlisted models: `claude-sonnet-5`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`, `gpt-5.5`, `gpt-5.4-mini`, `gpt-4o`. Visible text is capped at 8192 tokens. Claude Sonnet 5 turns adaptive thinking **off** (retries at `effort: low` with 16384 tokens if the API rejects that). GPT-5 requests use `reasoning_effort=none` and 16384 completion tokens (retries at `low` if needed) so a rubric is not swallowed by hidden reasoning.
